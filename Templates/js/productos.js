...
createApp({
  ...
  created() {
    ...
    document.getElementById("formularioBusqueda").addEventListener("submit", function (event) {
      ...
      var textoBuscado = document.getElementsByName("textoBuscado")[0].value.toLowerCase();
      if (textoBuscado.trim() !== "") {
        elVue.cargando = true;
    
        // Realizar una solicitud fetch para enviar los datos del formulario a la ruta Flask
        fetch("http://crod23.pythonanywhere.com/productos/find/")
          ...
          .then(function (data) {
            ...
            var productosFiltrados = data.filter(function (producto) {
              ...
              return nombreProducto.includes(textoBuscado);
            });
    
            // Mostrar los productos filtrados
            elVue.productos = productosFiltrados;
    ...
    document.getElementById("formularioBusqueda").addEventListener("submit", function (event) {
      ...
      if (textoBuscado !== "") {
        ...
        // Realizar una solicitud fetch para enviar los datos del formulario a la ruta Flask
        fetch("http://crod23.pythonanywhere.com/productos/find/" + textoBuscado)
          .then(function (response) {
            return response.json();
          })
          .then(function (data) {
  ...
  computed: {
    ...
    textoBoton() {
      var devolver = "-";
      if(this.tablaActual==1){
        devolver = `Items agregados a cotizacion (${this.productosCotizados.length})`;
      } else {
        devolver = `Items cotizados (${this.productosCotizados.length})`;
      }
      return devolver;
    }
  }
